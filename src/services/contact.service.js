...
const registerContact = async (contactDetails) => {
  ...
  if (
    [conName, conEmail, conPhoneNumber, conCompanyName, conSubject].some(
      (field) => field?.trim() === ""
    )
  ) {
  ...
  if (!savedContact) {
    throw new apiError(
      400,
      "Something went wrong, while registering the contact"
    );
  }
...
const deleteContact = async (contactId) => {
  validateObjectId(contactId, "Invalid contactId");
